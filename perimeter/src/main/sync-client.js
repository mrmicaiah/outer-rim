...
function createSync(options) {
  ...
  // Fetch the GitHub access token tied to this session. Lazy/cached —
  // only hits the worker once per app run, then returns the cached value.
  let cachedGithubToken = null;
  async function getGithubToken() {
    if (cachedGithubToken) return cachedGithubToken;
    if (!syncState.token) throw new Error('not_signed_in');
    const resp = await apiFetch('/v1/github-token');
    if (resp.status === 409) {
      // Old session minted before token storage — user needs to re-sign-in.
      throw new Error('stale_session_resign_in');
    }
    if (!resp.ok) throw new Error(`github_token_failed_${resp.status}`);
    const payload = await resp.json();
    cachedGithubToken = payload.githubAccessToken;
  ...
  async function signIn() {
    ...
    try {
      ...
      saveSyncState();
      cachedGithubToken = null; // Force refresh of GitHub token on next request
      setStatus('idle');
  ...
  return {
    init, signIn, signOut, syncNow, markDirty, pushToServer, pullFromServer, getGithubToken,
    getStatus: () => ({
