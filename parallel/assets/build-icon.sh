#!/bin/bash
# Build Parallel's macOS .icns icon from source.
# Run once from the parallel/ directory:  bash assets/build-icon.sh
# Produces: parallel/assets/icon.icns

set -e
cd "$(dirname "$0")"

# 1. Generate icon.png if it doesn't exist
if [ ! -f icon.png ]; then
