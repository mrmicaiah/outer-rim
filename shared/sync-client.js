...
// ============================================
//
// One module, four apps. Each app imports this from the main process.
//
...
//   });
//
// Then wire these handlers from the app's main.js:
//   await sync.init();
//   sync.markDirty();      // called by the app whenever state changes
//   await sync.signIn();   // opens OAuth flow
//   await sync.signOut();
//   await sync.syncNow();  // force a pull+push round trip
//
// Status values: 'idle' | 'signed-out' | 'pushing' | 'pulling' | 'error' | 'conflict'
...
function createSync(options) {
  ...
  // Persisted sync metadata: { token, githubLogin, githubUserId, etag, lastSyncedAt }
  ...
  async function pullFromServer() {
    ...
    if (resp.status === 401) {
      // Token was revoked server-side. Drop it locally.
  ...
  async function syncNow({ direction = 'auto' } = {}) {
    ...
    try {
      if (direction === 'pull' || !syncState.etag) {
  ...
  // Force-push on app quit. Best-effort — we can't await indefinitely.
  function wireQuitHook() {
    electronApp.on('before-quit', async (e) => {
      if (!dirty || !syncState.token) return;
      // Block quit briefly so the push has a chance.
      e.preventDefault();
      try {
        await Promise.race([
          pushToServer(),
  ...
  async function signIn() {
    if (syncState.token) {
      return { alreadySignedIn: true, githubLogin: syncState.githubLogin };
    }
    ...
    try {
      ...
      saveSyncState();
      setStatus('idle');
      // Immediate pull so the user's cloud state comes down.
  ...
  async function init() {
    ...
    // Pull on open so we pick up changes from other devices.
    if (syncState.token) {
      syncNow({ direction: 'pull' }).catch((err) =>
        console.error('[sync] startup pull failed:', err)
      );
    }
  }
...
// Spin up a tiny HTTP server on a free port in LOOPBACK_PORT_RANGE.
// Expects the OAuth callback page to POST { token, app, githubLogin, githubUserId } to /auth.
function openLoopbackServer() {
  return new Promise((resolve, reject) => {
    const tryPort = (port) => {
      if (port > LOOPBACK_PORT_RANGE[1]) {
        reject(new Error('no_free_loopback_port'));
        return;
      }
